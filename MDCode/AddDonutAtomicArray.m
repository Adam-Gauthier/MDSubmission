function AddDonutAtomicArray(orad,irad, X0, Y0, VX0, VY0, InitDist, Temp, Type)
global C
...
L = (2*orad - 1) * AtomSpacing;
W = (2*orad - 1) * AtomSpacing;

xp(1, :) = linspace(-L/2, L/2, 2*orad);
yp(1, :) = linspace(-W/2, W/2, 2*orad);

numAtoms = 0;
for i = 1:2*orad
    for j = 1:2*orad
        if (xp(i)^2 + yp(j)^2 <= (orad*AtomSpacing)^2) & (xp(i)^2 + yp(j)^2 >= (irad*AtomSpacing)^2)
            numAtoms = numAtoms+1;
